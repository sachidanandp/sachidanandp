<h1 align="center">
  Hey 👋, I'm Sachidanand
</h1>

<h3 align="center">
Android Engineer • Kotlin Enthusiast • Flutter
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=24&pause=1000&color=00C2FF&center=true&vCenter=true&width=900&lines=Android+Developer+with+7%2B+Years+of+Experience;Building+Scalable+%26+High+Performance+Apps;Jetpack+Compose+%7C+Kotlin+%7C+Clean+Architecture;Learning+System+Design+%26+DSA;Always+Learning+Something+New+%F0%9F%9A%80" />
</p>

---

## 👨‍💻 About Me

- 📱 Android Developer with **7+ years** of experience
- 🚀 Passionate about building scalable and high-performance mobile applications
- 💙 Love working with **Kotlin**, **Jetpack Compose**, and **Clean Architecture**
- 🏗️ Experienced in **MVVM**, **MVI**, Coroutines, Flow, and Modular Architecture
- ☁️ Azure Certified with basic cloud backend experience
- 🤖 Exploring AI integration in Android applications
- 🎯 Currently preparing for Product Companies & FAANG interviews

---

## 🚀 Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=kotlin,java,androidstudio,firebase,git,github,gradle,sqlite,mysql,postman,figma,vscode&perline=6"/>

</p>

### Android

- Kotlin
- Java
- Jetpack Compose
- MVVM, MVI
- Clean Architecture
- Hilt / Dagger
- Coroutines
- Flow, LiveData
- Room
- WorkManager
- Retrofit
- Glide
- Firebase
- REST APIs

---

## 📈 GitHub Stats

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=sachidanandp&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=sachidanandp&layout=compact&theme=tokyonight&hide_border=true"/>

</p>

---

## 🔥 GitHub Streak

<p align="center">

<img src="https://streak-stats.demolab.com?user=sachidanandp&theme=tokyonight&hide_border=true"/>

</p>

---

## 🏆 Achievements

- 📱 Developed enterprise-scale Android applications
- 🚀 Migrated legacy Java projects to Kotlin
- 🎨 Built modern UI with Jetpack Compose
- ⚡ Performance optimization using Android Profiler & Baseline Profiles
- 🔒 Implemented secure authentication & production-ready architectures
- ☁️ Azure Fundamentals Certified

---

## 📚 Currently Exploring

```text
📦 Android 17
🤖 AI in Android
🧠 DSA
🏗️ System Design
⚙️ Android Internals
📈 Performance Optimization
```

---

## 🌐 Connect with Me

<p align="left">

<a href="https://www.linkedin.com/in/sachidanand-pandit-93b938136/">
<img src="https://skillicons.dev/icons?i=linkedin"/>
</a>

<a href="mailto:spandit.sp11@gmail.com">
<img src="https://skillicons.dev/icons?i=gmail"/>
</a>

</p>

---

## 💡 Quote I Believe In

> **"First, solve the problem. Then, write clean and maintainable code."**

---

<p align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=sachidanandp&theme=tokyonight"/>

</p>

---

<p align="center">

<img src="https://komarev.com/ghpvc/?username=sachidanandp&label=Profile%20Views&color=0e75b6&style=flat"/>

</p>

---

<p align="center">
⭐️ If you like my work, consider giving a star to my repositories.
</p>

<p align="center">

<img src="https://raw.githubusercontent.com/sachidanandp/output/github-contribution-grid-snake.svg"/>

</p>
